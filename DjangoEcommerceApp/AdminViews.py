...
def registerPage(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			return redirect('admin_logout_process')
...
def test(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            mail = send_mail(form.cleaned_data['subject'], form.cleaned_data['content'], 'your_address',[recipient], fail_silently=True)
            if mail:
        ...
        form=ContactForm()
    return render(request, 'admin_templates/test.html', {'form':form}) 
